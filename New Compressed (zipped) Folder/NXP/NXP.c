...
void turn(void){
	...
	int derivativeMax1_index, derivativeMin1_index;
	float midpoint;
  
  derivativeMax1_index = 0;
  derivativeMin1_index = 127;
  
  //derive line exist
  for(i=0; i < 64; i++){
    current1 = derive_line[i]/*avg*/;
		current2 = derive_line[127 - i]/*avg*/;
  ...
  //find max
   ...
   //find min
    if((current2 < derive_line[derivativeMin1_index] && (127 - i) > 64) ){
		...
		SetServoDutyCycle(7.0-(((64.0 - midpoint)/64.0)*1.0), 50);
		SetMotorDutyCycle(35 - (((midpoint)/64.0)*10), 35 + (((midpoint-64.0)/64.0)*10), 10000, 1);
		
		
		/*
		if (midpoint - 64.0 > 5.0){
			SetServoDutyCycle(7.0-(midpoint/64.0), 50);
			SetMotorDutyCycle(35, 40, 10000, 1);
		}
		else if(64 - midpoint > 5.0){
			SetServoDutyCycle(7.0+(midpoint/64.0), 50);
			SetMotorDutyCycle(40, 35, 10000, 1);
		}
		else{
			SetServoDutyCycle(7.0, 50);
...
void calculateDerivatives(void){
	...
	for(i = 0; i < 128; i++ ){
		sum = ((1*line[i-2])+ (2*line[i-1]) + (4*line[i]) + (2*line[i+1]) + (1*line[i+2]));
		avg_line[i] = sum/5;
	}
